...
public class GoBackNSlidingWindow {
    ...
    public static int receiver(int start, int end) {
        // Simulate random ACK loss (30% chance)
        boolean ackLost = random.nextInt(10) < 3;

        if (ackLost) {
            System.out.println("Receiver: ACK lost! No ACK sent.");
            return -1; // sender must retransmit
        } else {
            int ack = end; // cumulative ACK
